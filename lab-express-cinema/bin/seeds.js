...
const dbmovies = 'lab-express-cinema';
mongoose.connect(`mongodb://localhost/${dbmovies}`)//conectar base de datos predefinida a mongo 
...
const movies = [
  ...
  //crear una base de datos .create utilizando el modelo Schema
  movie.create(movies, (error)=>{
      if (error){
        throw(error);  
      }
      console.log(`se ha creado la base con ${movies.length} elementos`);
